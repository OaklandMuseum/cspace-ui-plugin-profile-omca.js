export default {
  'about.title': 'CollectionSpace: Oakland Museum of California',

  'field.collectionobjects_common.objectNumber.name': 'Object number',
  'field.collectionobjects_common.distinguishingFeatures.name': 'Curatorial remarks',
  'field.collectionobjects_common.comment.name': 'Inhouse notes',
  'field.collectionobjects_common.materialComponent.fullName': 'Material note',
  'field.collectionobjects_common.materialComponent.name': 'Note',
  'field.collectionobjects_common.objectProductionPersonGroup.name': 'Maker',
  'field.collectionobjects_common.objectProductionPerson.fullName': 'Maker name',
  'field.collectionobjects_common.objectProductionPerson.name': 'Name',
  'field.collectionobjects_common.objectProductionPersonRole.fullName': 'Maker role',

  'panel.collectionobject.collect': 'Field Collection Information',
};
